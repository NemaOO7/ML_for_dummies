...
class InferentialCore:
    ...
    def __init__(self,X,y,regu=0,l1regu=0,tune = False):
        ...
        self.X =X
        self.y = y
        self.X_train ,self.y_train ,self.X_test ,self.y_test = train_test_split(self.X, self.y, test_size=0.33)
        self.regu = regu
        self.l1reg =l1regu
        self.tune = tune
    ...
    def startRegression(self,metric = r2_score,greater_isbetter = True):
        '''
        ...
        try:
                ...
                if self.regu == 0 and self.l1reg ==0:
                    ...
                    for i in halpha:
                        for j in hL1_wt:
                                ...
                                score.append([self.mertic(self.y_test,result.predict(self.X_test)),i,j])
                    score.sort(key=lambda x:x[0],reverse=self.greater_isbetter)
                    model = sm.OLS(self.y,self.X)
            ...
            logging.critical("Something went wrong"+e)
    ...
    def startLogistic(self,metric = accuracy_score, greater_isbetter = True):
        '''
        ...
        try:
            ...
            if(self.tune):
                ...
                if self.regu == 0 and self.l1reg ==0:
                    ...
                    for i in halpha:
                        for j in hL1_wt:
                                result =model.fit_regularized(alpha=i,L1_wt=j,refit = True)
                                score.append([self.mertic(self.y_test,[0 if result.predict(self.X_test)<=0.5 else 1 ]),i,j])
                    score.sort(key=lambda x:x[0],reverse=self.greater_isbetter)
                    model = sm.Logit(self.y,self.X)
                    result = model.fit_regularized(alpha=score[0][1],L1_wt=score[0][2],refit = True)
                else:
            ...
            logging.critical('Cannot converge if its PerfectSeparationError other wise :=='+e)
...
# y = sns.load_dataset('iris')['petal_width']
# =============================================================================
# 
# y = sns.load_dataset('iris')[["sepal_length"]]
...
# =============================================================================
# =============================================================================
# # BELOW DATA IS IMPORTANT
# =============================================================================
# =============================================================================
# =============================================================================
# 
# regressor = ['LinearRegression','Ridge','Lasso','DecisionTreeRegressor','SVR','KNeighborsRegressor','RandomForestRegressor','GradientBoostingRegressor','AdaBoostRegressor']
# classifier= ['KNeighborsClassifier','RandomForestClassifier','GradientBoostingClassifier','AdaBoostClassifier','DecisionTreeClassifier','SVC','LogisticRegression']
# regressor_dict = {}
# AdaBoostRegressor()
# exp = ['Ridge','Lasso','SVR']
# exp1 = ['DecisionTreeRegressor',"GradientBoostingRegressor"]
# for i in regressor:
#     if i in exp:
...
# #sv = SVLearningCore(X,y,{"SVR":{"kernel":[ "linear", "poly", "rbf", "sigmoid"],"C":[0.3,0.9,1.3]}},None,False,"high")
# sv = SVLearningCore(X,y,regressor_dict,None,False,"high")
# cu = (sv.startlearning())
# 
# =============================================================================
# =============================================================================
